...
# ----------------------------------------------------------------
Psi_t = torch.tensor(Psi_flat, dtype=torch.float32).view(-1, 1)   # target field Ψ⁺ (input)
Phi_t = torch.tensor(Phi_flat, dtype=torch.float32).view(-1, 1)   # source field Φ (target)
...
F_net = LearnableMapping(a_init=0.49, b_init=2.99)  # for example, “guess” values
...
Phi_I_pred = F_net(Psi_t).detach().numpy().flatten()   # informative field Φᴵ
Phi_R_pred = Phi_flat - Phi_I_pred                   # residual field Φᴿ


# After training, with F_net already trained

Psi_sample = Psi_t.detach().numpy().flatten()
Phi_I_learned = F_net(Psi_t).detach().numpy().flatten()
